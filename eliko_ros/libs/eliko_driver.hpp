  ...
  /* void getAnchorCoords(int clientSocket)
    ...
    int readData(int socket){
        ...
        while (true)
        {
            ...
            std::string mensaje;
            mensaje.append(getAnchorCommand);
            ...
            std::string mensajeTags;
            mensajeTags.append(getRRlCoordCommand);
            send(socket,mensaje.c_str(),mensajeTags.size(),0);
            while((bytesRead=recv(socket,buffer,sizeof(buffer),0))>0){
                std::vector<std::string> lines;
                char*line=std::strtok(buffer,"\n");
                while (line!=nullptr)
                ...
                for (auto& l:lines)
                {
                    ...
                    response.tagID=words[3];
                    int numOfWords=words.size();
                    int anchorData=numOfWords-4;
                    ...
                    for (int i = 0; i < anchorCoords; i++)
                    {
                        response.flags[i].flag=words[5+anchorCoords+1+i*2];
                    }
                    ...
                    response.tagCoords.Z=stoi(words[5]);
                    response.Info=words[6];
                    response.timestamp=stod(words[7]);
